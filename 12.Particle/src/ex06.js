...
export default function example6() {
  ...
  function setShape(e) {
    ...
    for (let i = 0; i < imagePannels.length; i++) {
      gsap.to(
        imagePannels[i].mesh.position,
        {
          duration: 2,
          x: array[i * 3],
          y: array[i * 3 + 1],
      ...
      if (type === 'random') {
        ...
        gsap.to(imagePannels[i].mesh.rotation, {
          duration: 2,
          x: imagePannels[i].sphereRotationX,
          y: imagePannels[i].sphereRotationY,
          z: imagePannels[i].sphereRotationZ
        });
      }
    }
  }
